 ...
 *avr-gcc -mmcu=atmega644p -DF_CPU=12000000 -Wall -Os -Wl,-u,vfprintf -lprintf_flt -lm  embedded_boost43.c -o embedded_boost43.elf  -Wl,-u,vfprintf -lprintf_flt -lm
 *avr-objcopy -O ihex embedded_boost43.elf embedded_boost43.hex
 *avrdude -c usbasp -p m644p -U flash:w:embedded_boost43.hex
 */
...
const uint8_t segments[2]= {0xFC,0x80};
...
void set_pwm(float v_input)
{
	...
	if((v_target * 0.99) > v_out && v_out > (v_target * 1.01)){
		duty = duty;
		}
