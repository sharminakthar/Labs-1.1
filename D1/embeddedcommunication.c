 ...
 *avr-gcc -mmcu=atmega644p -DF_CPU=12000000 -Wall -Os -Wl,-u,vfprintf -lprintf_flt -lm  embeddedcommunication.c -o embeddedcommunication.elf  -Wl,-u,vfprintf -lprintf_flt -lm
 *avr-objcopy -O ihex embeddedcommunication.elf embeddedcommunication.hex
 *avrdude -c usbasp -p m644p -U flash:w:embeddedcommunication.hex
 */
...
void set_pwm(float v_input)
{
 ...
 { 
	...
	while(1)
 {
    printf("."); // Repeatedly print dots to the screen
    if(kbhit()) // Check if there is an input available
    {
       scanf("%s", my_string); // Get the input
       printf("You said %s", my_string); // print the input to the screen
    }
 }
 }
